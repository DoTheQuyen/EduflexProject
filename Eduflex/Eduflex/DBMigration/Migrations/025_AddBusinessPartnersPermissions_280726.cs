...
{
    public class _025_AddBusinessPartnersPermissions_280726 : SafeMigrationBase
    {
        public override string MigrationId => "_025_AddBusinessPartnersPermissions_280726";
        public override string Name => "Add BusinessPartners Module Permissions";
        public override string Description => "Seeds the BusinessPartners module/permissions and grants access to Admin, Manager (full) and Staff (view only)";
        ...
        public override async Task Up(IMongoDatabase database)
        {
            ...
            var businessPartnersPermissionIds = await SeedModuleAndPermissions(modulesCollection, permissionsCollection, "BusinessPartners", "Business partner directory");

            await GrantPermissions(rolesCollection, adminRole["_id"].AsObjectId, businessPartnersPermissionIds.Values);
            Console.WriteLine("✅ Admin granted full BusinessPartners access");

            if (managerRole != null)
            {
                await GrantPermissions(rolesCollection, managerRole["_id"].AsObjectId, businessPartnersPermissionIds.Values);
                Console.WriteLine("✅ Manager granted full BusinessPartners access");
            }

            if (staffRole != null)
            {
                await GrantPermissions(rolesCollection, staffRole["_id"].AsObjectId, new[] { businessPartnersPermissionIds["View"] });
                Console.WriteLine("✅ Staff granted BusinessPartners view-only access");
            }
        }
        ...
        public override async Task Down(IMongoDatabase database)
        {
            ...
            var businessPartnersPermissions = await permissionsCollection.Find(
                Builders<BsonDocument>.Filter.In("key", new[]
                {
                    "BusinessPartnersView", "BusinessPartnersAdd", "BusinessPartnersEdit", "BusinessPartnersDelete"
                })).ToListAsync();

            var idsToRemove = businessPartnersPermissions.Select(p => p["_id"].AsObjectId.ToString()).ToList();
            ...
            await permissionsCollection.DeleteManyAsync(Builders<BsonDocument>.Filter.In("_id",
                businessPartnersPermissions.Select(p => p["_id"].AsObjectId)));

            await modulesCollection.DeleteManyAsync(Builders<BsonDocument>.Filter.Eq("name", "BusinessPartners"));

            Console.WriteLine("✅ Rolled back BusinessPartners catalog entries");
        }
    }
}
