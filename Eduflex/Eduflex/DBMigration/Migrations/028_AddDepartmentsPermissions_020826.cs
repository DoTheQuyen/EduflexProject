...
{
    public class _028_AddDepartmentsPermissions_020826 : SafeMigrationBase
    {
        public override string MigrationId => "_028_AddDepartmentsPermissions_020826";
        public override string Name => "Add Departments Module Permissions";
        public override string Description => "Seeds the Departments module/permissions and grants access to Admin, Manager (full) and Staff (view only)";
        ...
        public override async Task Up(IMongoDatabase database)
        {
            ...
            var departmentsPermissionIds = await SeedModuleAndPermissions(modulesCollection, permissionsCollection, "Departments", "Organizational department structure");

            await GrantPermissions(rolesCollection, adminRole["_id"].AsObjectId, departmentsPermissionIds.Values);
            Console.WriteLine("✅ Admin granted full Departments access");

            if (managerRole != null)
            {
                await GrantPermissions(rolesCollection, managerRole["_id"].AsObjectId, departmentsPermissionIds.Values);
                Console.WriteLine("✅ Manager granted full Departments access");
            }

            if (staffRole != null)
            {
                await GrantPermissions(rolesCollection, staffRole["_id"].AsObjectId, new[] { departmentsPermissionIds["View"] });
                Console.WriteLine("✅ Staff granted Departments view-only access");
            }
        }
        ...
        public override async Task Down(IMongoDatabase database)
        {
            ...
            var departmentsPermissions = await permissionsCollection.Find(
                Builders<BsonDocument>.Filter.In("key", new[]
                {
                    "DepartmentsView", "DepartmentsAdd", "DepartmentsEdit", "DepartmentsDelete"
                })).ToListAsync();

            var idsToRemove = departmentsPermissions.Select(p => p["_id"].AsObjectId.ToString()).ToList();
            ...
            await permissionsCollection.DeleteManyAsync(Builders<BsonDocument>.Filter.In("_id",
                departmentsPermissions.Select(p => p["_id"].AsObjectId)));

            await modulesCollection.DeleteManyAsync(Builders<BsonDocument>.Filter.Eq("name", "Departments"));

            Console.WriteLine("✅ Rolled back Departments catalog entries");
        }
    }
}
