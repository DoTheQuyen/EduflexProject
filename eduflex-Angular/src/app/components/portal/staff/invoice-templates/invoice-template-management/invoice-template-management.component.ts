...
export class InvoiceTemplateManagementComponent implements OnInit {
    ...
    { field: 'name', title: 'Template name' },
    {
      ...
      render: (_value, row: InvoiceTemplateDto) => {
        if (row.category === 'Customer') {
          return '<span class="badge-pill badge-pill-navy-soft">Customer</span>';
        }
        if (row.category === 'Partner') {
          return '<span class="badge-pill badge-pill-accent-soft">Partner</span>';
        }
    ...
    this.requestConfirm(
      ...
      () => {
        this.notificationService.success(
          `"${template.name}" copied — review the details and save to create the new template.`,
        );
        this.router.navigate(['/staff-portal/invoice-templates/new'], {
  ...
  openPreview(template: InvoiceTemplateDto): void {
    this.previewHtml = this.sanitizer.bypassSecurityTrustHtml(
      buildInvoiceTemplatePreviewHtml({
  ...
  private setStatus(template: InvoiceTemplateDto, isActive: boolean): void {
    ...
    this.requestConfirm(message, isActive ? 'Reactivate' : 'Deactivate', () => {
      this.client
        .statusPOST3(template.id!, new SetInvoiceTemplateStatusDto({ isActive }))
        .subscribe({
          next: () => {
            this.notificationService.success(
