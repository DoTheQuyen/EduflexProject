...
{
    public class _013_AddRolesAndUsersManagementPermissions_200726 : SafeMigrationBase
    {
        public override string MigrationId => "_013_AddRolesAndUsersManagementPermissions_200726";
        public override string Name => "Add Roles/Users Management Modules (Admin-only)";
        public override string Description => "Seeds Roles and Users modules/permissions and grants them to Admin only";
        ...
        public override async Task Up(IMongoDatabase database)
        {
            ...
            var rolesPermissionIds = await SeedModuleAndPermissions(modulesCollection, permissionsCollection, "Roles", "Role and permission management");
            var usersPermissionIds = await SeedModuleAndPermissions(modulesCollection, permissionsCollection, "Users", "User account management");

            await GrantPermissions(rolesCollection, adminRole["_id"].AsObjectId, rolesPermissionIds.Values);
            await GrantPermissions(rolesCollection, adminRole["_id"].AsObjectId, usersPermissionIds.Values);
            Console.WriteLine("✅ Admin granted Roles + Users management access");
        }
        ...
        public override async Task Down(IMongoDatabase database)
        {
            ...
            var rolesAndUsersPermissions = await permissionsCollection.Find(
            ...
            var idsToRemove = rolesAndUsersPermissions.Select(p => p["_id"].AsObjectId.ToString()).ToList();
            ...
            await permissionsCollection.DeleteManyAsync(Builders<BsonDocument>.Filter.In("_id",
                rolesAndUsersPermissions.Select(p => p["_id"].AsObjectId)));

            await modulesCollection.DeleteManyAsync(Builders<BsonDocument>.Filter.In("name", new[] { "Roles", "Users" }));

            Console.WriteLine("✅ Rolled back Roles/Users management catalog entries");
        }
    }
}
